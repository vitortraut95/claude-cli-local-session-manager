...
export function UsageDetailsModal({ session, open, onClose }: UsageDetailsModalProps) {
  ...
  return (
    <Modal open={open} title={session.title} onClose={onClose} size="lg">
      {models.length === 0 ? (
        <p className="text-sm text-stone-500 dark:text-stone-400">{t("usageDetailsModal.empty")}</p>
      ) : (
        <div className="flex flex-col gap-3">
          {models.map((model) => (
            <div
              key={model.model}
              className="rounded-lg border border-stone-100 bg-stone-50 p-3 dark:border-stone-800 dark:bg-stone-800/50"
            >
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-stone-600 dark:text-stone-400 sm:grid-cols-4">
                <span>
