    ...
    <Tooltip content={describeStatus(status, checking, updating, t)}>
      <span className="relative inline-flex">
        <Button
          ...
          className={`border ${
            updateAvailable
              ...
              : "border-stone-200 bg-marrom-50 text-stone-400 dark:border-stone-800 dark:bg-stone-800/50 dark:text-stone-600"
          }`}
          icon={
            isBusy ? (
        ...
        {updateAvailable && !isBusy && (
          <span className="absolute -top-1 -right-1 h-2.5 w-2.5 rounded-full bg-emerald-500 ring-2 ring-white dark:ring-stone-900" />
        )}
      </span>
    </Tooltip>
  );
