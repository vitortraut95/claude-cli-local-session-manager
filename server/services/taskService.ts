...
import { setNickname } from "../utils/nicknames.js";
 ...
 *  (`~/git` and `~/git/my-app` showing up as bare "git" and "my-app" gives no
...
export type RepoInfo = {
  ...
  /** Whether a Claude session is currently active with its working directory exactly at
   *  `repoRoot` (not a worktree under it) — the "no worktree" path checks the new branch out
   *  directly there, so this is the one case where skipping the worktree would actually collide
   *  with something. Drives the modal's smart default for the "don't use a worktree" checkbox:
   *  suggest a worktree only when this is true, default to no worktree otherwise. Same
   *  `workingDirectory === repoRoot && isActive` check `sessionService.ts` already uses for the
   *  equivalent guard in the worktree-to-root flows. */
  hasActiveSessionInRoot: boolean;
};

/** Used by the "new task" form to prefill the source-branch field once a folder is chosen/typed,
 *  and to compute the worktree-checkbox's smart default (see `RepoInfo.hasActiveSessionInRoot`). */
export async function getRepoInfo(folderPath: string): Promise<RepoInfo> {
  ...
  const [defaultBaseBranch, sessions] = await Promise.all([
    getDefaultBaseBranch(repoRoot),
    listSessions(),
  ]);
  const hasActiveSessionInRoot = sessions.some(
    (s) => s.workingDirectory === repoRoot && s.isActive,
  );
  return { repoRoot, defaultBaseBranch, hasActiveSessionInRoot };
}
 ...
 * `resolveBaseBranchRef` for why this prefers a freshly fetched `origin/<baseBranch>` unless
 * `useLocalBranch` (the modal's "local" source option) says to skip that and use the local branch
 * as-is.
 */
export async function resolveBaseBranch(
  ...
  const baseBranchRef = await resolveBaseBranchRef(repoRoot, trimmedBase, useLocalBranch);
  return { baseBranchRef };
}

/**
 * Best-effort: records `baseBranch` against `(repoRoot, branchName)` for the "Open PR" button's
 * later use (see `taskBaseBranches.ts`), but only when it actually differs from the repo's own
 * resolved default — branching off main/master (the common case) needs no entry, since that's
 * already what the default compare link assumes. Applies equally whether the task used a worktree
 * or checked the branch out directly in the repo root — a branch name can't collide with another
 * task's the way a shared root directory would, so both cases are safe to record. Never throws:
 * worst case the "Open PR" button just won't offer the creation-time base branch as an option, same
 * tolerance as `markWorktreeTrustAccepted`.
 */
async function recordTaskBaseBranchIfNonDefault(
  ...
  try {
    ...
    await recordTaskBaseBranch(repoRoot, branchName, trimmedBase);
  } catch {
 ...
 *
 * `useWorktree` is what the modal's "no worktree" checkbox resolves to (its smart default is now
 * *off* unless `RepoInfo.hasActiveSessionInRoot` says the root is busy — see `getRepoInfo` and
 * `NewTaskModal.tsx`): when false, this instead checks out the new
 ...
 * is needed for the two cases. The base branch is recorded the same way in both cases (see
 * `recordTaskBaseBranchIfNonDefault` — keyed by branch name, not worktree path, so it's safe either
 * way).
 */
export async function createTaskWorktree(
  ...
  if (!useWorktree) {
    ...
    await recordTaskBaseBranchIfNonDefault(repoRoot, trimmedBranch, baseBranch);
  ...
  //
  // Sanitizing collapses every non-`[A-Za-z0-9_-]` character (`/`, `.`, ...) to `-`, so two
  // *different* branch names (e.g. "feature/PROJ-123" and "feature-PROJ-123", both valid per
  // `SAFE_BRANCH_NAME` above) can land on the same worktree directory even though `branchExists`
  // found no collision for either one individually. The directory is still genuinely occupied
  // either way — `git worktree add` can't put two worktrees at one path — but which branch is
  // actually sitting there changes what the user needs to do about it, so look it up rather than
  // implying their own branch already has a worktree when it's really a different one blocking the slot.
  if (await directoryExists(worktreePath)) {
    const occupyingBranch = await getWorktreeBranch(worktreePath, repoRoot);
    const message =
      occupyingBranch && occupyingBranch !== trimmedBranch
        ? `The worktree folder for "${worktreeDirName}" is already in use by a different branch ` +
          `("${occupyingBranch}") — its name sanitizes to the same folder name. Rename this ` +
          `branch, or remove that worktree first.`
        : `A worktree named "${worktreeDirName}" already exists.`;
    throw new AppError("TASK_WORKTREE_EXISTS", message);
  }
...
export async function launchTaskTerminal(
  ...
  if (trimmedNickname) {
    ...
    await setNickname(newSessionId, trimmedNickname);
    sessionIdFlag = `--session-id ${newSessionId} `;
  }
